...
{
    public partial class edit_phancong : Form
    {
        private int iddoan;

        public edit_phancong(int iddoan, string hotennv, string chucvu, string ghichu)
        {
            this.iddoan = iddoan;
            InitializeComponent();
            comboBoxNV.DataSource = new BUS.NhanVienBUS().load_ds_hoten();

            comboBoxNV.Text = hotennv;
            comboBoxChucVu.Text = chucvu;
            textBox1.Text = ghichu;
        }
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            var res = new BUS.PhanCongBUS(iddoan).edit_phancong(idnhanvien, chucvu, ghichu);
            if (res > 0)
                this.Dispose();
            else if (res == 0)
